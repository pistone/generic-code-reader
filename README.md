...
# Step 1: Fetch tickets from Jira (saves to ticket_agent/tickets/):
python -m ticket_agent.fetch_tickets \
  --jql "project = PROJ AND status = Done AND updated >= -90d"

# Step 2: Re-run incrementally (only tickets updated since last fetch):
python -m ticket_agent.fetch_tickets \
  --jql "project = PROJ AND status = Done" --incremental

# Step 3: Extract knowledge and lessons from fetched tickets:
