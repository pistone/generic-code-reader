...
# Ticket knowledge — fetch then extract:
...
# Restrict processing to tickets whose key matches a pattern:
python -m ticket_agent.ticket_agent --tickets ticket_agent/tickets/ --key-pattern '^ABC-'
python -m ticket_agent.ticket_agent --tickets ticket_agent/tickets/ --key-pattern '^(ABC|DEF)-'
python -m ticket_agent.ticket_agent --tickets ticket_agent/tickets/   # all tickets

# Index lessons generated by the ticket agent (how-to guides from real fixes):
